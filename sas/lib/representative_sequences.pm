...
#  These variables are used as globals below.  They have been made SEED-aware,
#  but not SEED-dependent.  formatdb is run as a system command, and might
#  be encapsulated in a run() construct for the SEED.  blastall is in an
#  input pipe.

my $ext_bin = $FIG_Config::ext_bin;

my $formatdb = $ext_bin && -x "$ext_bin/formatdb" ? "$ext_bin/formatdb"
                                                  : "formatdb";
my $blastall = $ext_bin && -x "$ext_bin/blastall" ? "$ext_bin/blastall"
                                                  : "blastall";
...
{
    ref $_[0] eq 'ARRAY'
            or print STDERR "First parameter of rep_seq must be an ARRAY reference\n"
               and return undef;
    ...
    elsif ( @_ == 2 )
    {
        ...
        {
            print STDERR "Second parameter of rep_seq must be an ARRAY or HASH reference\n";
            return undef;
        }
    }
    ...
    elsif ( @_ == 3 )
    {
        if ( ref $_[1] ne 'ARRAY' )
        {
            print STDERR "Second parameter of 3 parameter rep_seq must be an ARRAY reference\n";
            return undef;
        }
        if ( ref $_[2] ne 'HASH' )
        {
            print STDERR "Third parameter of 3 parameter rep_seq must be a HASH reference\n";
            return undef;
        }
    ...
    {
        print STDERR "rep_seq called with @{[scalar @_]} parameters\n";
        return undef;
    }
    ...
    foreach ( keys %$options )
    {
        ...
        elsif ( m/size/i )                  #  add longest to shortest
        {
            $by_size = 1;
        }
        else
    ...
    if ( $by_size )
    {
        @$reps2 = sort { length( $b->[2] ) <=> length( $a->[2] ) } @$reps2;
    }
    ...
    foreach $entry ( @$seqs2 )
    {
        ...
        if ( $newdb )
        {
            make_blast_db( $db, $rep4blast, $protein );
            $newdb = 0;
        }
        ...
        if ( $tophit )
        {
            ...
            push @{ $group{ $gid } }, $id;  # add sequence to list in group
            $group_id{ $id } = $gid;       # record group for id
            print $logfile "$id\t$gid\n" if $logfile;
            ...
            if ( ! $tophit->[2] )
            {
                ...
                $max_bpp{ $id } = self_bpp( $db, $entry, $protein ) if $sim_meas =~ /^sc/;
                $newdb = 1;
            }
        }
    ...
    if    ( $save_tmp ) {}  #  This is not used, might be in future
    elsif ( $protein  ) { unlink $db, "$db.psq", "$db.pin", "$db.phr" }
...
sub rep_seq_2
{
    ...
    foreach ( keys %$options )
    {
        ...
        elsif ( m/by_?size/i || m/size/i )  #  add longest to shortest
        {
            $by_size = 1;
        }
        else
...
{   my ( $prog, $db, $qid, $qseq, $self, $blast_opt, $sort, $no_merge ) = @_;
    ...
    $cnt++;
    my ( $tmp_dir, $save_tmp ) = &tmp_dir( );
    $tmp_dir or print STDERR "Unable to locate temporary file directory\n"
             and return;
    my $query_file = "$tmp_dir/tmp_blast_seq_${$}_$cnt";
...
#  dependent manner:
#
#     ( $tmp_dir, $save_tmp ) = tmp_dir( \%options )
#
#  $save_tmp is true if the option 'savetmp' is true.
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
sub tmp_dir
{
