...
sub newick_desc_ref { ref($_[0]) ? $_[0]->[0] : Carp::confess() }  # = ${$_[0]}[0]
...
sub newick_n_desc {
    my $node = $_[0];
    array_ref( $node ) && array_ref( $node->[0] ) ? scalar @{ $node->[0] } : 0;
}

sub newick_desc_i {
    my ( $node, $i ) = @_;
    array_ref( $node ) && $i && array_ref( $node->[0] ) ? $node->[0]->[$i-1] : undef;
}
...
sub newick_tip_distances
{
    ...
    if ( ( $n_desc == 1 ) && $root && ( newick_lbl( $node ) ) )
    {
...
sub reroot_newick_between_nodes
{
    ...
    reroot_newick_between_nodes_by_path( $tree, \@path1, \@path2, $fraction )
}
...
sub reroot_newick_between_node_refs
{
    ...
    my @path1 = path_to_node_ref( $tree, $node1 ) or return undef;
    my @path2 = path_to_node_ref( $tree, $node2 ) or return undef;;
...
#
#  $tree = reroot_newick_between_nodes_by_path( $tree, $path1, $path2, $fraction )
#-------------------------------------------------------------------------------
sub reroot_newick_between_nodes_by_path
{
    my ( $tree, $path1, $path2, $fraction ) = @_;
    array_ref( $tree ) and array_ref( $path1 ) and  array_ref( $path2 )
       or return undef;
    $fraction >= 0 && $fraction <= 1 or return undef;
...
sub text_tree_row
{
    ...
    if ( @dl < 1 ) { push @$line, ( newick_lbl( $node ) || '' ), '' }
