...
=head3 submit_RAST_job
	...
	my $jobH = $rastObject->submit_RAST_job({
		...
		-taxonomyID => The NCBI taxonomy id   | Domain => [Archaea | Bacteria]
...
sub submit_RAST_job
{
	...
	if ($domain ne "archaea" && $domain ne "bacteria") {
		$error_string .= "-domain must be Archaea or Bacteria\n";
    	}
    }
    ...
    if ($filetype eq "fasta" && ! $tax_id) {
		if (!$genetic_code || ( $genetic_code != 4 && $genetic_code != 11)) {
			$error_string .= "-geneticCode must be 4 or 11\n";
		}
    }
